---
title: Clumping and Thresholding Windows by proxy
author: Sarah Urbut
date: 
output:
...
The C+T polygenic score is defined as the sum of allele counts (genotypes), weighted by estimated effect sizes ob tained from genome-wide association studies, where two filtering steps have been applied (Wray et al. 2007; Purcell et al. 2009; Dudbridge 2013; Wray et al. 2014; Euesden et al. 2014;
...
When applying C+T, one has three hyper-parameters to select, namely the squared correlation threshold $r_{2}$ and the window size $w_{c}$ of clumping, along with the p value threshold $p_{T}$. Usually, C+T users assign default values for clumping, such as $r_{c}$ of 0.1 (default of PRSice), 0.2 or 0.5 (default of PLINK), and $w_{c}$ of 250 kb (default of PRSice).
